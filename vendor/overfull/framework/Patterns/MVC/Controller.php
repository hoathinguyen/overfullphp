...
abstract class Controller extends BaseObject implements IController{
     ...
     * @return void
     */
    public final function validator(){
        if(!$this->validator){
            if($class = \Bag::route()->validator){
                // Get app root
                $appNamespace = Bag::config()->get('app.namespace');

                if(!class_exists($class)){
                    $class = "{$appNamespace}\\Validators\\{$class}";
                    if(!class_exists($class)){
                        throw new ValidatorNotFoundException($class);
                    }
                }

                $this->validator = new $class;
            } else {
                throw new UndefinedAttributeException("validator", "Route");
            }
        }
	...
	public final function run(){
            ...
            $actionPrefix = Bag::config()->get('mvc.action-prefix');
            $firstUC = Bag::config()->get('mvc.action-ucfirst');
            ...
            $dataTransfer = $this->{$__method}(Bag::request()->any());
